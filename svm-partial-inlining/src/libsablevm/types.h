...
typedef struct _svmt_stop_resume_struct _svmt_stop_resume;
...
typedef struct _svmt_mi_info_struct _svmt_mi_info; /* method inlining related info */
typedef struct _svmt_mi_invoke_info_struct _svmt_mi_invoke_info;

#if defined(_SABLEVM_DIRECT_THREADED_INTERPRETER)
typedef struct _svmt_mi_instruction_node_struct _svmt_mi_instruction_node;
#endif

#if defined(_SABLEVM_INLINED_THREADED_INTERPRETER)
typedef struct _svmt_mi_sequence_node_info_struct _svmt_mi_sequence_node_info;
#endif
...
typedef struct _svmt_mdebug_env_struct _svmt_mdebug_env;
typedef struct _svmt_mdebug_alloc_info_struct _svmt_mdebug_alloc_info_node;
typedef struct _svmt_mdebug_memblock_struct _svmt_mdebug_memblock;
#endif
...
#include "error_bits.h"
  ...
  struct _svmt_stop_resume_struct
  {
    jint last_index;
    struct
    {
...
#ifdef _SABLEVM_METHOD_INLINING
  /* The only usage of addr in locals if for JSR/ASTORE_RET/RET */
  unsigned used_as_addr:1;
...
struct _svmt_class_loading_struct
{
  ...
  _svmt_mi_instruction_node *instruction_tree;
#endif

#ifdef _SABLEVM_INLINED_THREADED_INTERPRETER
  _svmt_sequence_node *sequence_tree_by_bytecodes;
  _svmt_mi_sequence_node_info *sequence_tree_by_implementation;
#endif
...
struct _svmt_method_frame_info_struct
{
  ...
  /* number of non-parameter address local variables */
  jint non_parameter_addr_locals_count;
  jint non_addr_locals_count;

  _svmt_code *normal_code;
  _svmt_code *profiling_code;
  _svmt_code *selected_path;

  size_t code_length;
  size_t profiling_code_length;
  size_t selected_path_code_length;
  size_t nb_instructions; /* does not include parameters */

  _svmt_exception_table *selected_path_exception_table;
  jint selected_path_exception_table_nb_entries;
  ...
  /* extra space needed in stack_frame for inlined methods */
  size_t extra_frame_size;

  jboolean inlinable;

  jboolean has_been_inlined;
  jboolean contains_inlined_code;
  jboolean execution_is_in_caller_inlined_code;

  /* profiling info */
  size_t nb_executions;
  jint profiling_status;

  /* 'method_profiling offsets' for a given bytecode in the original code, 
     gives the new position in the profiling code array of this bytecode by 
     giving the offset from the beginning of the code profiling array */
  jint *mp_offsets;

  /* Given the offset (from the beginning of the code) of a bytecode in the 
     original code array, this table gives it's address in the current (newer)
     code array */
  _svmt_code *original_code_conversion_table;

#ifndef NDEBUG
  size_t original_code_conversion_table_length;
#endif
...
/*
----------------------------------------------------------------------
_svmt_mi_instruction_node
----------------------------------------------------------------------
*/
#if defined(_SABLEVM_DIRECT_THREADED_INTERPRETER)
struct _svmt_mi_instruction_node_struct
{
  jint instruction;
  void *implementation;

  _svmt_mi_instruction_node *parent;
  _svmt_mi_instruction_node *left;
  _svmt_mi_instruction_node *right;
};
#endif
...
*/
#if defined(_SABLEVM_INLINED_THREADED_INTERPRETER)
struct _svmt_mi_sequence_node_info_struct
{
  ...
  _svmt_mi_sequence_node_info *profiling_version;
/*   _svmt_mi_sequence_node_info *to_inline_version; */

  _svmt_mi_sequence_node_info *parent;
  _svmt_mi_sequence_node_info *left;
  _svmt_mi_sequence_node_info *right;
};
#endif

/*
----------------------------------------------------------------------
_svmt_mi_invoke_info
----------------------------------------------------------------------
*/
struct _svmt_mi_invoke_info_struct
{
  /* number of times this invoke site has been visited */
  size_t count;

  /* the last method that has been called from this site */
  _svmt_method_info *last_method;
};

/*
----------------------------------------------------------------------
_svmt_mi_info: Method Inlining Related Info.
----------------------------------------------------------------------
*/
struct _svmt_mi_info_struct
{
#ifndef NDEBUG
  _svmt_u64 inst_count;
  _svmt_u64 inst_count_times;
  _svmt_method_info *debug_method;
  _svmt_code *tmp_pc;
#endif

  /* number of times a method can be executed before being profiled */
  size_t method_profiling_start_threshold;

  /* number of times a method is executed before profiling stops */
  size_t method_profiling_stop_threshold;

  /* number of times an invoke site can be visited before being a 
     candidate for inlining */
  size_t invoke_inlining_threshold;

  /* will will inline callee methods only in calling methods with
   a number of bytecodes lower to this value */
  size_t max_calling_method_length;

  /* all methods with a number of bytecodes lower or egal to this 
     value will be potentially inlined */
  size_t max_callee_method_length;

  /* working arrays */

  /* used to store the code that will be inlined in the calling method */
  _svmt_code *selected_path_working_array;

  /* contains to original address of every instruction in the selected path */
  _svmt_code *selected_path_to_original_code_map;
  
  /* For each instruction of the selected path, this array contains 
     the address in the new calling method code array were the instruction 
     was placed */
  _svmt_code *selected_path_to_new_code_map;

  /* size of the selected path & addresses working arrays */
  size_t selected_path_working_arrays_length;
  
  /* Difference in length between the orginal calling code array
     and the new one */
  jint code_length_diff;

  /* invoke offset in the calling method code before inlining */
  jint invoke_offset;

  /* actual size of the code in the selected_path array */
/*   size_t last_method_inlined_code_length; */

  _svmt_exception_table *working_exception_table;
  jint working_exception_table_length;
  
  /*   jint tmp_exception_table_nb_entries; */

  jint invoke_instruction;	/* current type of invoke bytecode */

  /* indicates where to set the pc after inlining */
  _svmt_code *resuming_pc;
  
#ifdef _SABLEVM_INLINED_THREADED_INTERPRETER
  
  /* used by _svmh_mi_get_sequence() */
  _svmt_sequence_node *sequence;

/* Working array used before the merging of sequences
   to identify which sequences should be head of the new
   merged sequences */
  jboolean *sequence_heads;
  size_t sequence_heads_length;

  _svmt_code *merged_sequences_working_code;
  size_t merged_sequences_working_code_length;
  
  /* sequence containing the inline_invoke<X> where we
     should resume after inlining */
  _svmt_mi_sequence_node_info *resuming_seq;

  /* extra sequence put at the end of inlined callee_method */
/*   jint extra_sequence_offset; */
#endif

#ifndef NDEBUG
  /* used for debugging */
  jboolean show_instructions;
  FILE *code_arrays;
#endif

#if defined (_SABLEVM_METHOD_INLINING_STATS)

  size_t total_inlined_invokestatic;
  size_t total_inlined_invokespecial;
  size_t total_inlined_invokevirtual;
  size_t total_inlined_invokeinterface;

  size_t total_inlined_invokestatic_complete_sequence;
  size_t total_inlined_invokespecial_complete_sequence;
  size_t total_inlined_invokevirtual_complete_sequence;
  size_t total_inlined_invokeinterface_complete_sequence;

  size_t total_inlined_invokestatic_failed;
  size_t total_inlined_invokespecial_failed;
  size_t total_inlined_invokevirtual_failed;
  size_t total_inlined_invokeinterface_failed;

  size_t total_inlined_invokestatic_used;
  size_t total_inlined_invokespecial_used;
  size_t total_inlined_invokevirtual_used;
  size_t total_inlined_invokeinterface_used;
...
struct _svmt_JavaVM_struct
{
  ...
  jboolean verbose_method_inlining;
  jboolean verbose_method_profiling;
#endif
...
#ifdef _SABLEVM_METHOD_INLINING
  _svmt_exception_table *normal_exception_table;
  _svmt_exception_table *profiling_exception_table;
#endif
...
struct _svmt_stack_frame_struct
{
  ...
  size_t end_offset;
  _svmt_method_frame_info *method_frame_info;
  ...
  /* implementation address where we should jump to resume
     executing after inlining (might be in the middle of a 
     sequence) */
  const void *resuming_implementation;
#endif
};
...
/*
----------------------------------------------------------------------
_svmt_mdebug_memblock
----------------------------------------------------------------------
*/

struct _svmt_mdebug_memblock_struct
{
  _svmt_mdebug_alloc_info_node *large_block;
  _svmt_mdebug_memblock *next;
};

/*
----------------------------------------------------------------------
_svmt_mdebug_alloc_info
----------------------------------------------------------------------
*/

struct _svmt_mdebug_alloc_info_struct
{
  void *ptr;
  _svmt_u64 line;
  jint nb_bytes;

  _svmt_mdebug_alloc_info_node *parent;
  _svmt_mdebug_alloc_info_node *left;
  _svmt_mdebug_alloc_info_node *right;
};
...
struct _svmt_mdebug_env_struct
{
  FILE *alloc_history;
  FILE *results_file;

  _svmt_u64 history_curr_line;

  _svmt_u64 alloc_count;
  _svmt_u64 free_count;

  _svmt_u64 alloc_bcount;
  _svmt_u64 freed_bcount;
  _svmt_u64 unfreed_bcount;

  _svmt_mdebug_alloc_info_node *alloc_info_tree;

  /* for alloc_history and history_curr_line */
  pthread_mutex_t history_file_mutex;

  /* for alloc_info_arr, alloc_array_size and alloc_count */
  pthread_mutex_t addalloc_mutex;

  /* Java program exited with system exit */
  jboolean sys_exit;

  /* to prevent daemon threads at the end to modify data structures 
     by doing allocations during finalization */
  jboolean mdebug_enabled;

  /* used for custom allocations of _svmt_mdebug_alloc_info_node structures */
  _svmt_mdebug_memblock *memblock_list;

  jint current_memblock_offset;
  _svmt_mdebug_memblock *current_memblock;
};
