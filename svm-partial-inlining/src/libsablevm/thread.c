...
/*
-----------------------------------------------------
_svmf_thread_destroy: free memory used by the JNIEnv 
-----------------------------------------------------
*/

svm_static jint
_svmf_thread_destroy (_svmt_JNIEnv *env)
{
  _svmt_native_ref *next_locref;
  _svmt_native_ref *current_locref;
  ...
  current_locref = env->native_locals.list;

  while (current_locref != NULL)
    {
      next_locref = current_locref->next;
      _svmm_gzfree_native_ref_no_exception (current_locref);
      current_locref = next_locref;
    }

  current_locref = env->native_locals.free_list;
  ...
  if (env->stack.start != NULL)
    {
      _svmm_free (env->stack.start);
    }

  _svmm_cond_destroy (env->wakeup_cond);
  _svmm_cond_destroy (env->suspension.cond);
  _svmm_cond_destroy (env->contention.requester.cond);
  _svmm_mutex_destroy (env->contention.owner.mutex);

  _svmm_gzfree_env_no_exception (env);
...
/*
----------------------------------------------------------------------
_svmf_thread_recycle: put back the native local references in the
                      global free_list and put the JNIEnv in the free
                      list
----------------------------------------------------------------------
*/

svm_static jint
_svmf_thread_recycle (_svmt_JNIEnv *env)
{
  _svmt_JavaVM *vm = env->vm;
  _svmt_native_ref *next_locref;
  _svmt_native_ref *current_locref;
  ...
  while (current_locref != NULL)
    {
      ...
      /* put it in the global free_list */
      current_locref->ref = NULL;
      current_locref->previous = NULL;
      current_locref->next = vm->native_globals.free_list;

      if (vm->native_globals.free_list != NULL)
	{
	  vm->native_globals.free_list->previous = current_locref;
	}
      vm->native_globals.free_list = current_locref;

      current_locref = next_locref;
    }
  env->native_locals.list = NULL;
  ...
  env->native_locals.free_list = NULL;

  if (env->stack.start != NULL)
    {
      _svmm_free (env->stack.start);
      env->stack.start = NULL;
      env->stack.end = NULL;
      env->stack.current_frame = NULL;
    }

  env->thread.is_daemon = JNI_FALSE;
  env->thread.sleeping_on_fat_lock = NULL;
  env->thread.interrupted_status = SVM_THREAD_NOT_INTERRUPTED;
  env->critical_section_count = 0;
  env->status_flags = SVM_THREAD_STATUS_FLAGS_NONE;
  env->suspension.suspender_list = NULL;
  env->suspension.suspender_list_next = NULL;
  env->contention.owner.flag = 0;
  env->contention.owner.wait_list = NULL;
  env->contention.requester.wait_list_next = NULL;

#if defined(_SABLEVM_SIGNALS_FOR_EXCEPTIONS) || defined(_SABLEVM_INLINABILITY_TESTING)

  env->signal_handler = NULL;
  env->signal_code = SVM_SIGNAL_NONE;

#ifndef NDEBUG
  env->sigsegv_expected = 0;
  env->sigfpe_expected = 0;
#endif

#ifdef _SABLEVM_INLINABILITY_TESTING
  memset (&(env->inlinability_testing), 0,
	  sizeof (_svmt_inlinability_testing));
#endif

#endif

#ifndef NDEBUG
  memset (&(env->stop_resume), 0, sizeof (_svmt_stop_resume));
#endif

  /* The current thread has already been removed from the
     user or system thread list */

  /* put the JNIEnv in the free_list */
  env->previous = NULL;
  env->next = vm->threads.free_list;
  if (vm->threads.free_list != NULL)
    {
      vm->threads.free_list->previous = env;
    }
  vm->threads.free_list = env;
