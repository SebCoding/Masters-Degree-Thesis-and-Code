/* This file has been automatically generated from "vm_args.m4.c" */
  ...
  /* copying method_inlining parameters */
     
  else if (strncmp ("-Dsablevm.method_inlining.method_profiling_start_threshold=", optstr, 59) == 0)
    {
      if (_svmf_parse_size_t (&vm->mi_info.method_profiling_start_threshold, &optstr[59]) != JNI_OK)
	{
	  goto error;
	}
    }

  else if (strncmp ("-Dsablevm.method_inlining.method_profiling_stop_threshold=", optstr, 58) == 0)
    {
      if (_svmf_parse_size_t (&vm->mi_info.method_profiling_stop_threshold, &optstr[58]) != JNI_OK)
	{
	  goto error;
	}
    }

  else if (strncmp ("-Dsablevm.method_inlining.invoke_inlining_threshold=", optstr, 52) == 0)
    {
      if (_svmf_parse_size_t (&vm->mi_info.invoke_inlining_threshold, &optstr[52]) != JNI_OK)
	{
	  goto error;
	}
    }

  else if (strncmp ("-Dsablevm.method_inlining.max_calling_method_length=", optstr, 52) == 0)
    {
      if (_svmf_parse_size_t (&vm->mi_info.max_calling_method_length, &optstr[52]) != JNI_OK)
	{
	  goto error;
	}
    }

  else if (strncmp ("-Dsablevm.method_inlining.max_callee_method_length=", optstr, 51) == 0)
    {
      if (_svmf_parse_size_t (&vm->mi_info.max_callee_method_length, &optstr[51]) != JNI_OK)
	{
	  goto error;
	}
    }
