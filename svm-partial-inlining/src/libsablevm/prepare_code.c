...
_svmh_gzalloc_stack_and_local_map (__FILE__, __FUNCTION__, __LINE__, env, method, &ptr)

svm_static jint
_svmh_gzalloc_stack_and_local_map (const char *file, const char *func,
				   int line, _svmt_JNIEnv *env,
				   _svmt_method_info *method,
				   _svmt_stack_and_local_map **ptr)
{
  _svmt_stack_and_local_map *tmp;
  size_t size = sizeof (_svmt_stack_and_local_map);
  size +=
				   ...
				   _svmt_stack_and_local_map **ptr)
{
  ...
  tmp =
    _svmm_calloc (1, sizeof (_svmt_stack_and_local_map) +
		  (method->data.code_attribute->max_stack +
		   method->data.code_attribute->max_locals) * sizeof (jint));
...
_svmh_gzfree_stack_and_local_map (__FILE__, __FUNCTION__, __LINE__, &ptr)

svm_static void
_svmh_gzfree_stack_and_local_map (const char *file, const char *func,
				  int line, _svmt_stack_and_local_map **ptr)
{
#ifndef NDEBUG
...
#else /* not _SABLEVM_MEMORY_DEBUGGING */
...
#endif /* not _SABLEVM_MEMORY_DEBUGGING */
...
_svmf_compute_stack_gc_maps (_svmt_JNIEnv *env, _svmt_method_info *method)
{
  ...
  _svmt_stack_and_local_map *to_free_list = NULL;
	...
	case SVM_INSTRUCTION_RET:
	  {
	    ...
	    /* put back subroutine calls on work list (stack) making
	    ...
	    /* remerber stack_and_local_map to be freed at the end */
	    instruction->stack_and_local_map->free_list_next = to_free_list;
	    to_free_list = instruction->stack_and_local_map;
  ...
  /* Put the remaining _svmt_stack_and_local_map structures in the free list */
  {
    _svmt_stack_and_local_map *next;

    while (to_free_list != NULL)
      {
	next = to_free_list->free_list_next;

	to_free_list->free_list_next =
	  vm->class_loading.method_preparation.free_list;
	vm->class_loading.method_preparation.free_list = to_free_list;
...
#ifdef _SABLEVM_METHOD_INLINING
    method->prepared_info.non_addr_locals_count = next_local;

    /* assign indices for address local after non_ref locals */
    for (i = 0; i < locals_size; i++)
      {
	if (locals[i].used_as_addr)
	  {
	    locals[i].addr_index = next_local++;
	  }
      }

    method->prepared_info.non_parameter_addr_locals_count =
      next_local - method->prepared_info.non_addr_locals_count;

#if 0
    /* DEBUG */
    {
      jint j;
      _svmf_print_method_name (env, stderr, method, "AFTER SPLITTING: ");
      _svmf_printf (env, stderr, "[");
      for (i = 0; i < next_local; i++)
	{
/* 	  _svmf_printf(env, stderr, "("); */
	  for (j = 0; j < locals_size; j++)
	    {
	      if (locals[j].non_ref_index == i)
		{
		  if (locals[j].method_arg_type != 0)
		    {
		      _svmf_printf (env, stderr, "*");
		    }
		  _svmf_printf (env, stderr, "NR, ");
		}
	      if (locals[j].long_index == i)
		{
		  if (locals[j].method_arg_type != 0)
		    {
		      _svmf_printf (env, stderr, "*");
		    }
		  _svmf_printf (env, stderr, "L, ");
		}
	      if (locals[j].ref_index == i)
		{
		  if (locals[j].method_arg_type != 0)
		    {
		      _svmf_printf (env, stderr, "*");
		    }
		  _svmf_printf (env, stderr, "R, ");
		}
	      if (locals[j].addr_index == i)
		{
		  if (locals[j].method_arg_type != 0)
		    {
		      _svmf_printf (env, stderr, "*");
		    }
		  _svmf_printf (env, stderr, "A, ");
		}
	    }
	...
	case SVM_INSTRUCTION_ASTORE_3:
	  {
	    if (instruction->addr_stack_top)
	      {
#ifdef _SABLEVM_METHOD_INLINING
		switch (locals[instruction->data.arg.index].addr_index)
#else
		switch (locals[instruction->data.arg.index].non_ref_index)
#endif
		  {
...
_svmf_identify_sequences (_svmt_JNIEnv *env, _svmt_method_info *method)
{
  ...
  /* calls & all bytecode not inlinable */
  for (i = 0; i < instruction_count; i++)
    {
      _svmt_instruction_preparation *instruction = &instructions[i];
  ...
  for (i = 0; i < instruction_count; i++)
      ...
      if (instruction->is_sequence_head)
	{
	  ...
	  if (within_initialization_sequence)
	    {
	      /* must count the SKIP instruction at the begining of the sequence */
	      instruction->code_length = 2;
	    }
	  else
	    {
	      /* we coutn one for the super instruction */
	      instruction->code_length = 1;
	    }
	}
      else
	{
	  /* the instructions composing the sequence don't count 
	     since the sequence is only one super instruction */
      ...
      /* count parameters for the calculation of code_length & 
         calculate prepare_offset and prepare_length for every instruction */
...
#ifdef _SABLEVM_METHOD_INLINING
	    /* we are not profiling backward jumps, no more counter here */
/* 	    profiling_extra++; */
#endif
	  }
	  break;
...
#ifdef _SABLEVM_METHOD_INLINING
  method->prepared_info.code_length = offset + extra;
  method->prepared_info.profiling_code_length =
    offset + extra + profiling_extra;

#ifdef _SABLEVM_INLINED_THREADED_INTERPRETER
  method->prepared_info.preparation_sequences_offset = offset;
#endif
...
/* Create sequence_node and insert it in the sequence_tree if it is not 
   already present. Also returns through **psequence_implementation the
   address of the sequence implementation. This functions also copies 
   the implementations of the instructions into memory */

#if defined (_SABLEVM_INLINED_THREADED_INTERPRETER)

#define _svmm_get_sequence(env, instruction, sequence_implementation) \
_svmh_get_sequence (env, instruction, &sequence_implementation)

svm_static jint
_svmh_get_sequence (_svmt_JNIEnv *env,
		    _svmt_instruction_preparation *instruction,
		    void **psequence_implementation)
{
  ...
  if (vm->class_loading.method_preparation.sequence != NULL)
      ...
      if (vm->class_loading.method_preparation.sequence->instructions != NULL)
	{
	  ...
	  NULL)
	{
	  _svmm_free (vm->class_loading.method_preparation.sequence->
		      implementation);
...

  {
	...
	vm->class_loading.method_preparation.sequence->implementation =
	  _svmm_malloc (instruction->sequence_size);
	...
	_svmm_tree_insert_sequence_by_bytecodes (vm->class_loading.
						 ...
						 method_preparation.sequence);
	{
	  _svmt_mi_sequence_node_info *sequence_copy;
	  ...
	  /* We use this field to remember if the sequence contains the SKIP instruction
	     at its beginning. We don't need the implementation length */
	  if (instruction->sequence_contains_resolution)
	    {
	      sequence_copy->starts_with_skip = JNI_TRUE;
	    }
	  else
	    {
	      sequence_copy->starts_with_skip = JNI_FALSE;
	    }
...
/* 	  printf("inserted[%p] ", sequence_copy->implementation); */
/* 	  _svmf_print_super_instruction(env, stdout, sequence_copy->implementation); */
/* 	  printf("\n"); */
	}
#else
...
_svmf_compute_code_array (_svmt_JNIEnv *env, _svmt_method_info *method)
{
  ...
  for (i = 0; i < instruction_count; i++)
...
#ifdef _SABLEVM_METHOD_INLINING
	      method->prepared_info.nb_instructions++;
#endif
	    }
	  else
	    {
		  ...
		  (env, instruction, sequence_implementation) != JNI_OK)
		{
		  return JNI_ERR;
		}
...
#ifdef _SABLEVM_METHOD_INLINING
		code[extra++].jint =
		  locals[instruction->data.arg.index].addr_index;
#else
		code[extra++].jint =
		  locals[instruction->data.arg.index].non_ref_index;
#endif /* SABLEVM_METHOD_INLINING */
	      }
	...
	case SVM_INSTRUCTION_INVOKEINTERFACE:
	  {
	    ...
	    {
	      jint inst;
	      _svmt_code tmp = code[instruction->code_offset];
	      ...
	      inst =
		_svmf_mi_get_instruction_int_code (env, tmp.implementation);
#endif
		...
		case SVM_INSTRUCTION_INVOKESTATIC:
		  {
#if SVM_PROFILE_AND_INLINE_INVOKESTATIC
		    code[extra++] =
		      vm->instructions[SVM_INSTRUCTION_INITIAL_INVOKESTATIC].
		      code;
#else
		    code[extra++] =
		...
		case SVM_INSTRUCTION_INVOKESPECIAL:
		  {
#if SVM_PROFILE_AND_INLINE_INVOKESPECIAL
		    code[extra++] =
		      vm->instructions[SVM_INSTRUCTION_INITIAL_INVOKESPECIAL].
		      code;
#else
		    code[extra++] =
		...
		case SVM_INSTRUCTION_INVOKEVIRTUAL:
		  {
#if SVM_PROFILE_AND_INLINE_INVOKEVIRTUAL
		    code[extra++] =
		      vm->instructions[SVM_INSTRUCTION_INITIAL_INVOKEVIRTUAL].
		      code;
#else
		    code[extra++] =
		...
		case SVM_INSTRUCTION_INVOKEINTERFACE:
		  {
#if SVM_PROFILE_AND_INLINE_INVOKEINTERFACE
		    code[extra++] =
		      vm->
		      instructions[SVM_INSTRUCTION_INITIAL_INVOKEINTERFACE].
		      code;
#else
		    code[extra++] =
		      vm->instructions[SVM_INSTRUCTION_INVOKEINTERFACE].code;
#endif
		    break;
		  }
		default:
      ...
      if (instruction->is_sequence_last_resolution)
	{
	  ...
	  assert ((first->code >= 0)
		  && (first->code < SVM_INSTRUCTION_COUNT));
...
#if SVM_PROFILE_AND_INLINE_INVOKESTATIC
		code[extra++] =
		  vm->instructions[SVM_INSTRUCTION_INITIAL_INVOKESTATIC].code;
#else
		code[extra++] =
		  vm->instructions[SVM_INSTRUCTION_INVOKESTATIC].code;
#endif
...
#if SVM_PROFILE_AND_INLINE_INVOKEINTERFACE
		code[extra++] =
		  vm->instructions[SVM_INSTRUCTION_INITIAL_INVOKEINTERFACE].
		  code;
#else
		code[extra++] =
		  vm->instructions[SVM_INSTRUCTION_INVOKEINTERFACE].code;
...
/* 
----------------------------------------------------------------------
_svmf_mi_init_conversion_table
----------------------------------------------------------------------
*/

svm_static jint
_svmf_mi_init_conversion_table (SVM_UNUSED _svmt_JNIEnv *env,
				_svmt_method_info *method)
{
  size_t i;
  _svmt_code *table;
  _svmt_method_frame_info *frame_info = &(method->prepared_info);
  _svmt_code *code = frame_info->code;
  size_t length = frame_info->code_length;

  if (_svmm_cl_malloc_code (env, method->class_info->class_loader_info,
			    length, table) != JNI_OK)
    {
#ifndef NDEBUG
      _svmf_printf (env, stderr, "method inlining: memory alloc failed\n");
#endif
      return JNI_ERR;
    }

  frame_info->original_code_conversion_table = table;

#ifndef NDEBUG
  frame_info->original_code_conversion_table_length = length;
#endif
...
#ifdef _SABLEVM_METHOD_INLINING
  method->data.code_attribute->normal_exception_table = table;
#endif
...
_svmf_prepare_code (_svmt_JNIEnv *env, _svmt_method_info *method)
{
  ...
  /* Not inlinable by default */
#if defined (_SABLEVM_METHOD_INLINING)
  method->prepared_info.inlinable = JNI_FALSE;
#endif
  ...
  /* Evaluate if method is inlinable */
#if defined (_SABLEVM_METHOD_INLINING)

#if 1
  if(method->prepared_info.nb_instructions <= vm->mi_info.max_callee_method_length)
    {
      method->prepared_info.inlinable = JNI_TRUE;
    }
#endif

/*   if(_svmf_mi_method_is(method, "org/sablecc/sablecc/DFA", "eclosure")) */
/*     { */
/*       env->vm->mi_info.debug_method = method; */
/*       _svmf_printf(env, stderr, "eclosure prepared\n"); */
/*     } */
...
/*     } */
  ...
  if ((_svmf_is_set_flag (method->access_flags, SVM_ACC_NATIVE)) ||
      (strcmp (DREF (method->name, value), "<clinit>") == 0))
    {
      method->prepared_info.inlinable = JNI_FALSE;
    }

#if 0 
  if (_svmf_mi_method_is (method, "HelloWorld", 
			  "m1"))
    {
      method->prepared_info.inlinable = JNI_TRUE;
      _svmf_printf (env, stderr, "%s.%s is inlinable\n",
		    method->class_info->name, DREF (method->name, value));
    }
#endif

#if 0
  if(method->prepared_info.inlinable)
    {
      _svmf_printf (env, stderr, "%s.%s is inlinable\n",
		    method->class_info->name, DREF (method->name, value));
    }
  ...
  if(method->prepared_info.inlinable)
  {
    if (_svmf_mi_init_conversion_table (env, method) != JNI_OK)
      {
	goto error;
      }
  }
...
#ifdef _SABLEVM_METHOD_INLINING
  method->frame_info->normal_code = method->frame_info->code;
#endif
...
fix_frame:
  {
    ...
    frame_data.method_frame_info = &method->prepared_info;
...
/*   printf("Preparation of %s finished\n", DREF (method->name, value)); */
