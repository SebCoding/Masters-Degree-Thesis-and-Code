...
*/
...
#ifdef _SABLEVM_METHOD_INLINING
	{
	  ...
	  if(_svmf_mi_init_sequence_by_implementation_tree(env, SVM_INSTRUCTION_$1, 
							   implementation_length)
	     != JNI_OK)
	    {
	      return JNI_ERR;
...

	{
	  ...
	  _svmf_mi_debug_on_dispatch_ptr (env, stack_size, pc, "$1");
#endif
	  /* instruction body */
	  $3 m4_dnl;
...
*/
...
#ifdef _SABLEVM_METHOD_INLINING
	{
	  ...
	  if (_svmm_gzalloc_mi_instruction_node (env, node) != JNI_OK)
	    {
	      return JNI_ERR;
	    }
	  ...
	  node->implementation =
	    (void *) env->vm->instructions[instr].code.implementation;

	  _svmm_tree_insert_instructions (env->vm->class_loading.
					  instruction_tree, node);

/* 	  printf("inserted[%d][%p]\n", instr, node->implementation); */
	}
#endif
	...
	/* dispatch */
#if defined (_SABLEVM_METHOD_INLINING) && !defined(NDEBUG)
	_svmf_mi_debug_on_dispatch (env, stack_size, pc, "$1");
#endif
	goto *((pc++)->implementation);
      }
      m4svm_define_end = ":])";
