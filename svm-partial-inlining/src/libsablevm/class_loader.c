...
/*       _svmf_printf(env, stderr, "super_class[%s]\n", */
/* 		   DREF (DREF (class->super_class, name), value)); */

/*       _svmf_printf(env, stderr, "_svmf_bootcl_derive_class: [%s] != [%s]\n", class_name, class->name); */
/*       _svmf_printf(env, stderr, "_svmf_bootcl_derive_class: failed for [%s] 4\n", class_name); */
      return JNI_ERR;
    }
   ...
   jboolean *free_after_use)
{
  ...
  if (_svmm_gmalloc_cchars (env, strlen (class_name) + strlen (".class") + 1,
  ...
  _svmm_gmfree_cchars (class_file_name);
   ...
   const char *class_name, _svmt_class_file *class_file)
{
  ...
  {
    size_t path_length =
      strlen (class_path) + strlen ("/") + strlen (class_name) +
      strlen (".class") + 1;
    char *path;

    _svmm_gmalloc_cchars (env, path_length, path);
...

  {
	...
	if (entry->is_file == JNI_FALSE)
	  {			/* directory - look for an uncompressed .class file */
	    ...
	    struct stat stats;
	    size_t path_length = strlen ("file://") +
	      strlen (entry->class_path) + strlen ("/") + strlen (res_name) +
	      1;
	    char *path;
	    ...
	    if (stat (path + strlen ("file://"), &stats) == 0)
	      {			/* we found the file! */
		return path;
	      }
	    else
	  ...
	  {			/* file - means .jar or .zip usually */
		...
		size_t path_length = strlen ("jar:file://") +
		  strlen (entry->class_path) + strlen ("!/") +
		  strlen (res_name) + 1;
		char *path;
...
_svmf_cl_destroy (_svmt_class_loader_info *class_loader_info)
{
  _svmt_memory_block *current;
  _svmt_memory_block *tmp = class_loader_info->memory_block_list;

  while ((current = tmp) != NULL)
    {
      tmp = current->next;
      _svmm_free (current->block);
      _svmm_gzfree_memory_block (current);
    }
