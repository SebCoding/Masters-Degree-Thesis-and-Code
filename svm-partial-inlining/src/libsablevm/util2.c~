...
/*
----------------------------------------------------------------------
_svmf_tree_compare_instructions
----------------------------------------------------------------------
*/
#if defined (_SABLEVM_METHOD_INLINING) && defined (_SABLEVM_DIRECT_THREADED_INTERPRETER)
inline svm_static jint
_svmf_tree_compare_instructions (_svmt_mi_instruction_node * value1,
				 _svmt_mi_instruction_node * value2)
{
  assert (value1 != NULL);
  assert (value2 != NULL);

  if (value1->implementation > value2->implementation)
    {
      return 1;
    }
  if (value1->implementation < value2->implementation)
  ...
  /* incorrect on 64 bits platforms */
  /* return (char*)value1->implementation - (char*)value2->implementation; */
...
/*
----------------------------------------------------------------------
_svmf_tree_compare_sequence_by_bytecodes
----------------------------------------------------------------------
*/

inline svm_static jint
_svmf_tree_compare_sequence_by_bytecodes (_svmt_sequence_node *value1,
...
/*   _svmf_printf(NULL, stderr, "\ncompare: value1->instructions_length=%d\n", */
/* 	       value1->instructions_length); */

/*   _svmf_printf(NULL, stderr, "\ncompare: value2->instructions_length=%d\n", */
/* 	       value2->instructions_length); */
...
/*   _svmf_printf(NULL, stderr, "\ncompare: 3\n"); */
  ...
  while (diff == 0 && i < size)
    {
/*       _svmf_printf(NULL, stderr, "compare in while: [%d] [%d] size[%d]\n", */
/* 		   value1->instructions[i], value2->instructions[i], size); */
...
/*
----------------------------------------------------------------------
_svmf_tree_compare_sequence_by_implementation
----------------------------------------------------------------------
*/

inline svm_static jint
_svmf_tree_compare_sequence_by_implementation (_svmt_mi_sequence_node_info *
					       value1,
...
/*
----------------------------------------------------------------------
_svmf_mi_copy_sequence_node_to_sequence_node_info
----------------------------------------------------------------------
*/

svm_static jint
_svmf_mi_copy_sequence_node_to_sequence_node_info (_svmt_JNIEnv *env,
						   _svmt_sequence_node
						   *to_copy,
						   _svmt_mi_sequence_node_info
						   ** result)
{
  ...
  if (_svmm_gzalloc_mi_sequence_node_info (env, *result) != JNI_OK)
    {
      return JNI_ERR;
    }

  if (_svmm_gmalloc_ints
      (env, to_copy->instructions_length, (*result)->instructions) != JNI_OK)
    {
      return JNI_ERR;
    }

  (*result)->instructions_length = to_copy->instructions_length;

  for (i = 0; i < to_copy->instructions_length; i++)
    {
      (*result)->instructions[i] = to_copy->instructions[i];
    }

  (*result)->implementation = to_copy->implementation;
  (*result)->implementation_length = to_copy->implementation_length;
...
_svmf_tree_splay_gc_map_free_internal (_svmt_gc_map_node *node)
{
  if (node != NULL && node->bits != NULL)
    _svmm_gzmfree_ubytes (node->bits);
}
...
svm_static void SVM_UNUSED
_svmf_tree_splay_sequence_free_internal (_svmt_sequence_node *node)
{
  if (node != NULL)
    {
      if (node->instructions != NULL)
	{
	  _svmm_gmfree_ints (node->instructions);
	}

      if (node->implementation != NULL)
	{
	  _svmm_free (node->implementation);
	}
    }
}
#endif
...
svm_static void SVM_UNUSED
_svmf_tree_splay_sequence_info_free_internal (_svmt_mi_sequence_node_info *
					      node)
{
  if (node != NULL)
    {
      if (node->instructions != NULL)
	{
      ...
      /* already freed in the destruction of the sequence by butecode tree */
/*       if (node->implementation != NULL) */
/* 	{ */
/* 	  _svmm_free (node->implementation); */
/* 	} */
  ...
  if (env->vm->verbose_methods == JNI_TRUE)
	...
	env->stack.current_frame->method_frame_info->method;
      ...
      _svmf_printf (env, stderr,
		    "T%d: [verbose methods: entering method %s.%s%s pc[%p]]\n",
		    env->thread.id,
